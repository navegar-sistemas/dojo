...
export const decisions: IArchitectureDecision[] = [
  {
    key: "ADR-041",
    title: "Bug #1 (deadlock no-op): fix permanece na 013; a 017 adiciona um guard UNITÁRIO e a prova de CENA real foi ENTREGUE (follow-up integrado)",
    context:
      "O deadlock do turno sem animação (game_controller aguarda animations_finished, mas all_done era emitido SÍNCRONO antes do await em turnos no-op/pivot/parede/atacar-vazio) já foi corrigido na 013 (AnimationSequencer.play() difere all_done um frame quando nenhum tween foi aguardado, commit 6ccc57c), integrada na main. Há teste UNITÁRIO do mecanismo na 013 (test_all_done_nao_e_sincrono_sem_animacao). O PO desejava, além do unitário, uma prova de RUNTIME no nível de cena (render-rule) — ENTREGUE como follow-up (test_017_noop_render_cena.gd) e integrada na main (677d1c4).",
    decision:
      "NÃO duplicar o fix. A 017 referencia o fix/teste da 013 e adiciona um GUARD UNITÁRIO do invariante do all_done diferido (test_017_noop_cena.gd) — equivalente/co-localizado ao test_all_done_nao_e_sincrono_sem_animacao da 013; é um teste UNITÁRIO do AnimationSequencer e NÃO instancia game.tscn nem dirige game_controller. A prova de NÍVEL DE CENA real (instanciar game.tscn / dirigir game_controller num turno no-op, renderizar ≥1 frame e asserir o avanço, honrando a render-rule) foi ENTREGUE como follow-up (test_017_noop_render_cena.gd, dev 42warrior, aceito pelo PO e integrado na main 677d1c4). Nenhuma lógica de produção nova entra pelo #1.",
    consequences:
      "Cobertura do #1 completa: nível de MECANISMO (sequencer) pelo guard unitário + nível de CENA integrada pelo render-test entregue. Sem retrabalho do fix. Honestidade: a spec distingue o guard unitário (T-170) da prova de cena real (follow-up entregue), sem alegação falsa.",
    status: "accepted",
    requirementKeys: ["RF-170", "RNF-170"],
    rejectedAlternatives: [
      {
        alternative: "Reabrir a 013 / reaplicar o fix dentro da 017",
        reason: "O fix já está vivo e testado na main; reaplicar duplicaria fonte e arriscaria regressão. A prova de cena de runtime foi entregue como follow-up (test_017_noop_render_cena.gd), não como re-edição do fix.",
      },
    ],
  },
  {
