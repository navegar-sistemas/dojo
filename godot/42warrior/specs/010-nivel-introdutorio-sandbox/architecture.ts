...
export const decisions: IArchitectureDecision[] = [
  {
    ...
    consequences:
      "Todo o motor de turnos, entidades e editor de código reusados sem alteração (RNF-100). O fluxo pós-conclusão diverge apenas num if is_sandbox na camada de progressão (RF-102). A GameScreen precisa de uma Label condicional para hint_text (RF-103). Custo: LevelData ganha dois campos opcionais; progressão ganha a branch de sandbox.",
    status: "accepted",
    requirementKeys: ["RF-101", "RF-102", "RF-103", "RF-104", "RNF-100"],
    rejectedAlternatives: [
      {
        alternative: "Criar uma SandboxScreen separada com cena própria",
        reason:
          "Duplica a lógica da GameScreen (arena, editor, HUD, console de turnos), aumenta superfície de manutenção e viola o requisito de reutilização de sistemas (RNF-100) sem nenhum benefício funcional.",
      },
    ],
  },
  {
    ...
    consequences:
      "Integração limpa com a feature 009 sem novo componente (RF-100). O LevelRegistry define o sandbox como level_id=0 e o ProgressionManager sabe que level_id=0 nunca é bloqueado — uma exceção de 2 linhas no método de verificação de desbloqueio.",
    status: "accepted",
    requirementKeys: ["RF-100", "RF-104"],
    rejectedAlternatives: [
      {
...
export const components: IComponent[] = [
  ...
  },
  {
    ...
    requirementKeys: ["RF-100", "RF-102", "RF-104"],
  },
  {
