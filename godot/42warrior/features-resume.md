...
- [x] **016 — Fidelidade visual animada (glitch/42)** · spec `done` · **COMPLETA na main** (Sprints 1+2+3+4 — fundações + telas P0 + telas P1 + delta-A animações)
  ...
  - **Sprint 3 — 6/6 telas P1 INTEGRADAS + FECHADO** (count-level render-proof, aceite PO reviewer=usuario; merge `5a4308a`, done-marking `237a23b`): **T-166** resultado · **T-167** transição · **T-168** sandbox · **T-169** glossário · **T-170** API-ref · **T-171** conclusão + **correção factual dos créditos** (atribuição CC0 falsa removida → "Arte e áudio originais"). O count-level pegou gap REAL: as telas não chamavam `build_theme()` → fonte default; corrigido na produção. check.sh **403/403**.
  - **Sprint 4 — delta-A (RF-160 amplia rescue/captive: hero_rescue/captive idle/rescue via registro)** `future` — PO sequenciou pós-telas-P1/018-apresentação (US-173/T-173).

- [x] **018 — Mundo bidimensional + warrior 4 direções** · spec **`done`** · **VERTICAL 2D COMPLETA — domínio + API + apresentação, 3 sprints FECHADOS e INTEGRADOS** (main `9115d93`) · 461/461
  - **Sprint 1 (domínio 2D, P0)** — `78ab331`: grade R×C (corredor 1×N = caso especial R=1, retrocompat por construção). **T-180** LevelState 2D + **T-181** Direction 4-dir + **T-182** Space/sentidos 2D + coverage gaps (guard 1D/2D no warrior_facade). Domínio puro, 0-RNG determinista.
  - **Sprint 2 (API 2D, P1)** — `9017967`/done-marking `dceab3d`: **T-183** WarriorApiCatalog.direction_entries (4 cardinais + forward/backward + pivot) + glossário "directions". **RETROCOMPAT exemplar**: `warrior_facade` INTOCADO, scripts beginner 1×N verdes sem mudar uma linha do código do jogador.
  - **Sprint 3 (apresentação 2D, P2)** — merge `ca137a6`/done-marking `9115d93`: **T-184** TileMap R×C renderizado (count-level `get_used_cells()>0`) + câmera (011) seguindo nos 2 eixos + animações virar/andar 4-dir (flip_h/rotation por direção) · **T-185** LevelLoader 2D R×C (1×N=R=1, reference_solutions jogam) · **T-186** não-regressão. Aceite PO render-rule "exemplar". O mundo 2D agora é **visível e jogável**.
...
- [x] **022 — Robustez contra blocos inline/one-liner** · spec `done` · **INTEGRADA na main** (`4b3f920` código + `c8713b3` spec, push) — **P0 reportado de novo pelo Matheus, FECHADO**
  - O depth/loop-guard da 020 só cobria blocos **multi-linha**: **`func play_turn(w): play_turn(w)`** (recursão one-liner) e **`while c: x()`** inline ESCAPAVAM → Stack overflow → crash. Reproduzido em headless. Fix: **`_normalize_inline_blocks()`** quebra corpos inline em multi-linha (ciente de parênteses/strings) ANTES da instrumentação → os guards existentes passam a cobri-los. +4 testes de regressão (one-liner recursão + while inline). Rastreado por PR-010 (RF-220/US-220/T-220). check.sh **403/403**.

- [x] **021 — Export do jogo para o navegador (Web/HTML5)** · spec Sprint 1 `closed` · **T-210..214 TODOS done** · rastreia PR-014
  - **SMOKE WEB VERDE** (`ed9b683`): Playwright headless chromium executou smoke real — WASM 38MB carregou, canvas renderizando (WebGL2 ativo), FPS 120 (alvo ≥30), user:// IndexedDB `/userfs` populado (persistência), COOP/COEP headers OK, 0 erros. T-214 done → US-212 fechada → Sprint 1 021 fechado. check.sh desktop 403/403 verde (0-regressão). (Para GitHub Pages: build `nothreads` ou `coi-serviceworker`, pois Pages não envia COOP/COEP.)
...
Evolução do tema: o glitch deixa de ser só efeito e vira **ferramenta** que o jogador detecta e explora a favor. Cadeia aprovada pelo PO (cmqxyjjd8); GO de ativação dado pelo PO (cmqy5xkbx) — gates 016 (done) + 018 (done) satisfeitos. Sprint 1 `active`; **T-190 + T-191 INTEGRADOS** (`852180a`, 494/494), **T-192 (GlitchExploits) em HANDOFF ao 42warrior**. Reusa GlitchRule/GlitchPostProcess (015) + sentidos 2D/LevelState (018). Fonte de verdade: `specs/019-glitch-como-ferramenta-do-jogador-exploits-a-favor/`.
...
- [x] **T-191** GlitchDetection — `Space.glitch_active()/glitch_next()` públicos; `LevelState.with_glitch_model` (CQS, não-muta); `glitch_next=window_open(seed,turn+1)` dá **≥1 turno de antecedência**; aditivo (null=legado). **INTEGRADO `852180a`** (merge feature/019-sprint1-t191, check.sh 494/494, aceite PO cmqy7d2pp, +14 testes; 3-way merge preservou done-marking T-190)
- [ ] **T-192** GlitchExploits — 4 mecânicas de domínio: no-clip seedado · buffer-overflow→dano dobrado · memory-leak→cura dobrada · inimigo intangível piscante (efeito só na janela) · _em HANDOFF ao dev (cmqy7k8f0)_
