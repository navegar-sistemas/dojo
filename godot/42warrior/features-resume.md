...
- [~] **021 — Export do jogo para o navegador (Web/HTML5)** · spec `implementing` · **T-210..213 INTEGRADOS + done** (merge `cf26d61`, done-marking `cdf70c3`) · rastreia PR-014
  - **HTML EXPORT FUNCIONANDO**: export templates Godot 4.7 instalados (`~/Library/Application Support/Godot/export_templates/4.7.stable/`) + `godot --headless --export-release "Web"` gera `index.html`/`js`/`wasm` (38M)/`pck`. Build versionado em `godot/42warrior/build/web` (`97c7a6f`), roda no navegador (servir com headers COOP/COEP). **T-210** renderer `gl_compatibility` · **T-211** preset Web · **T-212** audio-gesture gate · **T-213** persistência IndexedDB — done, aceite PO. **T-214 (smoke web REAL — warrior roda ≥1 turno do código do jogador no WASM no browser) `todo`, DESBLOQUEADO** — dev implementa; é o gate de valor que fecha a 021. (Para GitHub Pages: build `nothreads` ou `coi-serviceworker`, pois Pages não envia COOP/COEP.)
